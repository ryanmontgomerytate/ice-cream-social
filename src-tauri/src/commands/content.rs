use crate::database::{
    models::{
        AudioDrop, AudioDropInstance, ChapterLabelRule, ChapterType, Character,
        CharacterAppearance, EpisodeChapter, FlaggedSegment, SegmentClassification, Sponsor,
        TranscriptCorrection, TranscriptCorrectionWithEpisode,
    },
    Database, DetectedContent, DetectedContentWithEpisode, SearchResult, TranscriptSegment,
};
...
#[tauri::command]
pub async fn get_chapter_types(db: State<'_, Arc<Database>>) -> Result<Vec<ChapterType>, AppError> {
...
#[tauri::command]
pub async fn delete_chapter_type(db: State<'_, Arc<Database>>, id: i64) -> Result<(), AppError> {
...
pub async fn create_episode_chapter(
    ...
    log::info!(
        "Creating chapter for episode {}: type={}, start={}",
        episode_id,
        chapter_type_id,
        start_time
    );
...
pub async fn delete_episode_chapter(
    ...
    db.delete_episode_chapter(chapter_id)
        .map_err(AppError::from)
}
...
#[tauri::command]
pub async fn get_characters(db: State<'_, Arc<Database>>) -> Result<Vec<Character>, AppError> {
...
#[tauri::command]
pub async fn delete_character(db: State<'_, Arc<Database>>, id: i64) -> Result<(), AppError> {
...
#[tauri::command]
pub async fn get_sponsors(db: State<'_, Arc<Database>>) -> Result<Vec<Sponsor>, AppError> {
...
#[tauri::command]
pub async fn delete_sponsor(db: State<'_, Arc<Database>>, id: i64) -> Result<(), AppError> {
...
pub async fn add_sponsor_mention(
    ...
    log::info!(
        "Adding sponsor {} mention in episode {}",
        sponsor_id,
        episode_id
    );
...
pub async fn search_transcripts(
    ...
    log::info!(
        "Searching transcripts for: '{}' (limit={}, offset={})",
        query,
        limit,
        offset
    );
    ...
    let results = db
        .search_transcripts(clean_query, limit, offset)
        .map_err(AppError::from)?;
    let total = db
        .count_search_results(clean_query)
        .map_err(AppError::from)?;
...
#[tauri::command]
pub async fn get_search_stats(db: State<'_, Arc<Database>>) -> Result<SearchStats, AppError> {
    let indexed_segments = db.get_indexed_segment_count().map_err(AppError::from)?;
    let unindexed_episodes = db.get_unindexed_episodes().map_err(AppError::from)?;
...
pub async fn index_all_transcripts(
    ...
    let episodes = db.get_unindexed_episodes().map_err(AppError::from)?;
    ...
    for (i, episode) in episodes.iter().enumerate() {
        ...
        let content = match std::fs::read_to_string(transcript_path) {
            ...
            Err(e) => {
                log::warn!(
                    "Failed to read transcript for episode {}: {}",
                    episode.id,
                    e
                );
                failed += 1;
    ...
    log::info!(
        "Bulk indexing complete: {} indexed, {} failed, {} total",
        indexed,
        failed,
        total
    );
...
pub async fn reindex_all_with_speakers(
    ...
    let episodes = db
        .get_all_transcribed_episode_ids()
        .map_err(AppError::from)?;
    ...
    for (i, (episode_id, title)) in episodes.iter().enumerate() {
        ...
        match db.index_episode_from_file(*episode_id) {
            ...
            Ok(n) => {
                log::info!(
                    "Reindexed {} segments for episode {} ({})",
                    n,
                    episode_id,
                    title
                );
    ...
    log::info!(
        "Reindex-with-speakers complete: {} indexed, {} failed, {} total",
        indexed,
        failed,
        total
    );
...
pub async fn get_detected_content_by_type(
    ...
    db.get_detected_content_by_type(&content_type)
        .map_err(AppError::from)
}
...
pub async fn create_flagged_segment(
    ...
    log::info!(
        "Creating flagged segment for episode {} segment {}: type={}",
        episode_id,
        segment_idx,
        flag_type
    );
...
pub async fn get_flagged_segments(
    ...
    db.get_flagged_segments_for_episode(episode_id)
        .map_err(AppError::from)
}
...
#[tauri::command]
pub async fn delete_flagged_segment(db: State<'_, Arc<Database>>, id: i64) -> Result<(), AppError> {
...
pub async fn get_unresolved_flag_count(
    ...
    db.get_unresolved_flag_count(episode_id)
        .map_err(AppError::from)
}
...
pub async fn get_character_appearances_for_episode(
    ...
    db.get_character_appearances_for_episode(episode_id)
        .map_err(AppError::from)
}
...
pub async fn get_character_appearances_for_character(
    ...
    db.get_character_appearances_for_character(character_id)
        .map_err(AppError::from)
}
...
#[tauri::command]
pub async fn get_audio_drops(db: State<'_, Arc<Database>>) -> Result<Vec<AudioDrop>, AppError> {
...
pub async fn update_audio_drop_transcript(
    ...
    db.update_audio_drop_transcript(drop_id, &text)
        .map_err(AppError::from)
}
...
pub async fn update_audio_drop_window(
    ...
    db.update_audio_drop_window(drop_id, min_window, max_window)
        .map_err(AppError::from)
}

#[tauri::command]
pub async fn delete_audio_drop(db: State<'_, Arc<Database>>, id: i64) -> Result<(), AppError> {
...
pub async fn get_audio_drop_instances(
    ...
    db.get_audio_drop_instances_for_episode(episode_id)
        .map_err(AppError::from)
}
...
pub async fn auto_label_chapters(
    ...
    let existing = db
        .get_episode_chapters(episode_id)
        .map_err(AppError::from)?;
    ...
    let segments = db
        .get_transcript_segments_for_episode(episode_id)
        .map_err(AppError::from)?;
    if segments.is_empty() {
        return Err(AppError::Other(
            "No transcript segments indexed for this episode — run FTS index first".into(),
        ));
    }
    ...
    for rule in &enabled_rules {
        ...
        if let Some((seg_idx, _, start_time)) = matched {
            ...
            .map_err(AppError::from)?;
            log::info!(
                "Auto-labeled chapter type {} at segment {} for episode {}",
                rule.chapter_type_id,
                seg_idx,
                episode_id
            );
...
pub async fn run_ai_chapter_detection(
    ...
    let segments = db
        .get_transcript_segments_for_episode_full(episode_id)
        .map_err(AppError::from)?;
    if segments.is_empty() {
        return Err(AppError::Other(
            "No transcript segments indexed for this episode".into(),
        ));
    }
...
Respond with JSON only."
    ...
    let response = client
        .extract_content(
            &lines,
            &prompt,
            Some("You are a careful labeling assistant. Output JSON only."),
        )
        .await
    ...
    let json = response
        .parsed_json
        .ok_or_else(|| AppError::Other("AI response did not contain valid JSON".into()))?;
    ...
    for item in arr {
        ...
        let get_time = |idx: i32| -> (f64, f64) {
            ...
            let (start, end_opt) = segments
                ...
                .unwrap_or((0.0, None));
            let end = end_opt
                .or_else(|| segments.get(idx_usize + 1).map(|(_, _, s, _)| *s))
...
pub async fn export_sponsor_clip(
    ...
    let image_url = if !clean_num.is_empty() {
        Some(format!(
            "https://heyscoops.fandom.com/wiki/Special:FilePath/ICS_{}.png",
            clean_num
        ))
    } else {
        None
...
pub async fn run_qwen_classification(
    ...
    let home_dir =
        dirs::home_dir().ok_or_else(|| AppError::Other("Failed to get home dir".into()))?;
    let project_dir = home_dir
        .join("Desktop")
        .join("Projects")
        .join("ice-cream-social-app");
    let venv_python = project_dir.join("venv").join("bin").join("python");
    let classify_script = project_dir
        .join("scripts")
        .join("qwen_classify_segments.py");
    ...
    let audio_path = db
        .get_episode_audio_path(episode_id)
        .map_err(AppError::from)?
        .ok_or_else(|| {
            AppError::Other(format!(
                "Episode {} has no downloaded audio file — download it first",
                episode_id
            ))
        })?;
    ...
    let segments_json =
        serde_json::to_string(&target_segments).map_err(|e| AppError::Other(e.to_string()))?;
    let characters_json =
        serde_json::to_string(&char_json).map_err(|e| AppError::Other(e.to_string()))?;
    ...
    let result: serde_json::Value = serde_json::from_str(json_str.trim()).map_err(|e| {
        AppError::Other(format!(
            "Failed to parse qwen output JSON: {} — raw: {}",
            e,
...
pub async fn get_segment_classifications(
    ...
    db.get_segment_classifications(episode_id)
        .map_err(AppError::from)
}
...
pub async fn approve_segment_classification(
    ...
    db.approve_segment_classification(id)
        .map_err(AppError::from)
}
...
pub async fn run_qwen_polish(
    ...
    let result: serde_json::Value = serde_json::from_str(json_str.trim()).map_err(|e| {
        AppError::Other(format!(
            "Failed to parse qwen polish output: {} — raw: {}",
            e,
            &json_str[..json_str.len().min(300)]
        ))
    })?;
...
pub async fn get_transcript_corrections(
    ...
    db.get_transcript_corrections(episode_id)
        .map_err(AppError::from)
}
...
pub async fn approve_all_corrections_for_episode(
    ...
    log::info!(
        "Approving all pending corrections for episode {}",
        episode_id
    );
    db.approve_all_pending_corrections_for_episode(episode_id)
        .map_err(AppError::from)
}
...
pub async fn reject_all_corrections_for_episode(
    ...
    log::info!(
        "Rejecting all pending corrections for episode {}",
        episode_id
    );
    db.reject_all_pending_corrections_for_episode(episode_id)
        .map_err(AppError::from)
}
...
pub async fn get_episode_interaction_summary(
    ...
    db.get_episode_interaction_summary(episode_id)
        .map_err(AppError::from)
}

...
| Phase 3: Phone-first polish | `in_progress` | Responsive foundations present; PWA baseline shipped (manifest route, service worker registration, install CTA/instructions). | Run mobile UX performance/accessibility pass (tap targets, nav ergonomics, Lighthouse/mobile budgets) and finalize installability QA on iOS/Android. |
...
| Phase 3 PWA baseline | `done` | Added `manifest.webmanifest`, service worker registration (`/sw.js`), mobile install UI (`Install App`/`Install Guide`), and `/install` instructions page. |
...
2. Run Phase 3 mobile UX/accessibility pass and capture Lighthouse metrics + fixes.

...
| Phase 2: Community editing + moderation | `in_progress` | Hosted Phase 2 schema foundation migration added/applied (`profiles`, roles/memberships, revisions, pending edits, moderation queue/actions, reports, trust/rate-limit/audit, import batch items); first admin-gated read APIs shipped (`/api/v1/admin/revisions`, `/api/v1/admin/pending-edits`, `/api/v1/admin/moderation-queue`) plus `/admin` review dashboard surface. | Add authenticated role-aware write actions (approve/reject/assign), then apply RLS policies for non-service-key access paths. |
...
2. Add Phase 2 moderation write endpoints and role-aware approvals (approve/reject/assign).

...
| Phase 1: Web read + hosted model | `in_progress` | Supabase schema + import pipeline; hosted import/verify modes; web episodes/search/wiki/episode detail reads live; hosted verify job wired in Actions; wiki index/discovery route shipped. | Tighten search ranking relevance and tune result quality. |
...
1. Tighten search ranking relevance and tune result quality.

...
| Phase 1: Web read + hosted model | `in_progress` | Supabase schema + import pipeline; hosted import/verify modes; web episodes/search/wiki/episode detail reads live; hosted verify job wired in Actions; wiki index/discovery route shipped; ranked search RPC + fallback path implemented. | Apply migration in hosted env and validate ranking quality on real query samples. |
...
1. Apply ranked search migration in hosted env and validate result quality with real queries.
